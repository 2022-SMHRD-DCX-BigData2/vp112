...
public class BoardRestController {
	...
	@GetMapping("/ViBoard")
	public List<TblCommunity> ViboardAjaxList() { 
		List<TblCommunity> list = Bmapper.ViboardList();
		return list;
	}
	@PostMapping("/ViBoard")
	public void ViboardAjaxInsert(TblCommunity vo) {
		Bmapper.ViboardInsert(vo);
	}
	@DeleteMapping("/ViBoard/{idx}")
	public void ViboardAjaxDelete(@PathVariable int idx) {
		Bmapper.ViboardDelete(idx);
	}
	@PutMapping("/ViBoard")
	public void ViboardAjaxUpdate(@RequestBody Board vo) { 
		Bmapper.boardContentUpdate(vo);
	}
	//조회수 증가 
	@GetMapping("/ViupdateCount/{idx}")
	public TblCommunity ViupdateCount(@PathVariable int idx) {
		Bmapper.ViupdateCount(idx);
		//해당 idx를 가지는 게시글 내용
		TblCommunity vo = Bmapper.ViboardContent(idx);
